...
function getCategoryList()
{
		...
		/*plantons liste*/
	if($_GET['plantons']=="yes") {
	$sql = "SELECT cat_id, cat_name, cat_image
	        FROM tbl_category
			WHERE (cat_parent_id = 0 
			AND cat_name NOT LIKE '<!-- nondisp -->%')" //doit-on montrer la categorie?
			." AND cat_id = '39' " //on cache les plantons id cat=39
			."ORDER BY cat_name";
			#echo $sql; //test
	}else{
/*produits sur commande liste*/
	$sql = "SELECT cat_id, cat_name, cat_image
	        FROM tbl_category
			WHERE (cat_parent_id = 0 
			AND cat_name NOT LIKE '<!-- nondisp -->%')" //doit-on montrer la categorie?
			." AND cat_id NOT LIKE '39' " //on cache les plantons id cat=39
			."ORDER BY cat_name";
			#echo $sql; //test
}	
...
/*plantons liste*/
	if($_GET['plantons']=="yes") {
	        ...
	        FROM tbl_category 
            WHERE cat_id = '39' " //on cache les plantons id cat=39
			."ORDER BY cat_name";
	}else{
/*produits sur commande liste*/
    $sql = "SELECT cat_id, cat_parent_id, cat_name, cat_image, cat_description
	        FROM tbl_category 
            WHERE cat_id NOT LIKE '39' " //on cache les plantons id cat=39
			."ORDER BY cat_name";
}
#echo $sql; exit;
