...
function getPagingLink($sql, $itemPerPage = 10, $strGet = '')
{
	...
	if ($totalPages > 1) {
		...
		if ($pageNumber > 1) {
			$page = $pageNumber - 1;
			if ($page > 1) {
				$prev = " <a href=\"$self?page=$page&$strGet/\">[Précédent]</a> ";
			} else {
				#$prev = " <a href=\"$self?$strGet\">[Premier]</a> ";
			}	
				
			$first = " <a href=\"$self?$strGet\">[Premier]</a> ";
		} else {
		...
		if ($pageNumber < $totalPages) {
			$page = $pageNumber + 1;
			$next = " <a href=\"$self?page=$page&$strGet\">[Suivant]</a> ";
			$last = " <a href=\"$self?page=$totalPages&$strGet\">[Dernier]</a> ";
		} else {
