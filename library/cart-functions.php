...
function addToCart()
{
	...
	if (dbNumRows($result) != 1) {
		...
		if ($currentStock == 0) {
			...
			setError('Le produit souhaité n\'est pas disponible');
			header('Location: cart.php');
			exit;
		}
	...
	for ($i = 0; $i < $numItem; $i++) {
		...
		if ($newQty < 1) {
			...
			if ($newQty > $row['pd_qty']) {
				...
				if ($row['pd_qty'] > 0) {
					setError('Le produit que vous avez demandé n\'est pas disponible dans les quantités souhaitées.');
				} else {
					// the product is no longer in stock
					setError('Désolé, le produit que vous souhaitez (' . $row['pd_name'] . ') n\'est plus en stock');
